...
REQUIRES:
    pip install streamlit pandas pyarrow plotly scikit-learn xgboost shap
"""
...
# =============================================================================
# SEVERITY SCORE + SHAP HELPERS
# =============================================================================

def compute_severity(proba: np.ndarray) -> float:
    """
    Continuous 0–100 congestion severity score.
    score = 0*P(Low) + 50*P(Medium) + 100*P(High)
    No retraining needed — derived purely from class probabilities.
    """
    return float(0 * proba[0] + 50 * proba[1] + 100 * proba[2])
...
def get_shap_explainer(_model, model_name: str):
    ...
    Cached so it's only built once per model.
    """
...
def compute_shap_values(explainer, X_input: np.ndarray):
    """Return SHAP values array for a single input row."""
    if explainer is None:
        return None
    try:
        vals = explainer.shap_values(X_input)
        # vals shape: (n_classes, n_samples, n_features) for RF multiclass
        # or (n_samples, n_features, n_classes) for XGBoost
        if isinstance(vals, list):
            return np.array(vals)          # RF: list of arrays per class
        return vals
    except Exception:
        return None
...
with tab2:
    if fdf.empty:
        ...
        view_mode = st.radio(
            "View mode",
            ["Congestion labels", "Speed heatmap", "Density heatmap", "Severity score"],
            horizontal=True,
        )
        ...
        if view_mode == "Congestion labels":
            ...
            # Severity score map — compute score per row then average spatially
            sev_sample = map_sample.copy()
            if model is not None:
                feat_cols_map = load_feature_cols()
                available = [c for c in feat_cols_map if c in sev_sample.columns]
                if len(available) == len(feat_cols_map):
                    X_map = sev_sample[feat_cols_map].astype(float).values
                    probas = model.predict_proba(X_map)
                    sev_sample["severity"] = [compute_severity(p) for p in probas]
                else:
                    sev_sample["severity"] = (
                        sev_sample["congestion_label"]
                        .map({"Low": 10, "Medium": 50, "High": 90})
                        .fillna(50)
                    )
            else:
                sev_sample["severity"] = (
            ...
            fig_map = px.density_heatmap(
                sev_sample, x="x", y="y", z="severity",
                histfunc="avg", nbinsx=60, nbinsy=60,
                color_continuous_scale=["#1a5c3a", "#ffb300", "#ff5252"],
                range_color=[0, 100],
                labels={"x": "X (m)", "y": "Y (m)", "severity": "Severity (0–100)"},
                title="Congestion severity score heatmap  (0=free flow · 100=gridlock)",
            )
...
with tab4:
    ...
    if not feature_cols:
        ...
        if st.button("▶  PREDICT CONGESTION LEVEL"):
            ...
            # ── Severity score ──────────────────────────────────────────
            severity = compute_severity(proba)
            sev_col  = severity_color(severity)
            sev_lbl  = severity_label(severity)

            res_l, res_mid, res_r = st.columns([1, 1, 2])
            ...
            with res_mid:
                # Severity gauge as a single horizontal bar
                st.markdown(f"""
                <div class="pred-box">
                    <div class="pred-conf">SEVERITY SCORE</div>
                    <div class="pred-label" style="color:{sev_col}">{severity:.0f}<span style="font-size:1.2rem;color:#4a7fa5"> / 100</span></div>
                    <div class="pred-conf">{sev_lbl.upper()}</div>
                </div>
                """, unsafe_allow_html=True)

                # Thin progress-style bar
                bar_html = f"""
                <div style="margin-top:0.8rem;background:#1a2540;border-radius:6px;height:10px;overflow:hidden;">
                  <div style="width:{severity:.0f}%;background:{sev_col};height:100%;border-radius:6px;transition:width 0.4s;"></div>
                </div>
                <div style="display:flex;justify-content:space-between;font-family:'DM Mono',monospace;font-size:0.6rem;color:#2e4a6a;margin-top:3px;">
                  <span>0 free flow</span><span>50</span><span>100 gridlock</span>
                </div>
                """
            ...
            # ── SHAP explanation ─────────────────────────────────────────────
            st.markdown('<div class="section-title">SHAP feature explanation — why this prediction?</div>', unsafe_allow_html=True)

            if not SHAP_AVAILABLE:
                st.info("Install SHAP for feature explanations: `pip install shap`")
            else:
                # Use RF or XGBoost for SHAP (LR uses a different explainer)
                shap_model_name = sel_model if sel_model in ["Random Forest", "XGBoost"] else "Random Forest"
                shap_model      = load_model(shap_model_name)
                explainer       = get_shap_explainer(shap_model, shap_model_name)
                shap_vals       = compute_shap_values(explainer, X_input)

                if shap_vals is not None:
                    # Normalize SHAP output to shape (n_classes, n_features)
                    # so we can index by predicted class consistently.
                    #
                    # RF  → shap_values() returns list of n_classes arrays,
                    #        each shaped (n_samples, n_features)
                    #        → stack to (n_classes, n_samples, n_features)
                    #
                    # XGBoost → shap_values() returns a single array shaped
                    #        (n_samples, n_features, n_classes) when output_margin=False
                    #        OR (n_samples, n_features) for binary/single-output.
                    #        TreeExplainer on multiclass XGB returns
                    #        (n_samples, n_features, n_classes).
                    #
                    # We normalise everything to (n_classes, n_features).
                    try:
                        if isinstance(shap_vals, list):
                            # RF: list of (1, n_features) → stack → (n_classes, n_features)
                            sv_all = np.array([v[0] for v in shap_vals])  # (n_classes, n_features)
                        elif isinstance(shap_vals, np.ndarray):
                            if shap_vals.ndim == 3:
                                # XGBoost: (n_samples, n_features, n_classes)
                                sv_all = shap_vals[0].T   # → (n_classes, n_features)
                            elif shap_vals.ndim == 2:
                                # Single-output fallback: (n_samples, n_features)
                                sv_all = shap_vals[[0]]   # → (1, n_features), use class 0
                            else:
                                sv_all = shap_vals.reshape(1, -1)
                        else:
                            sv_all = np.array(shap_vals)

                        # Guard: if n_classes dimension is smaller than pred index, use 0
                        class_idx = pred if pred < sv_all.shape[0] else 0
                        sv = sv_all[class_idx]            # (n_features,)

                    except Exception as shap_err:
                        st.warning(f"Could not parse SHAP values: {shap_err}")
                        sv = None
                    ...
                    if sv is not None:
                        ...
                        feat_vals  = sv[top_idx]
                        bar_colors = [COLOR_HIGH if v > 0 else COLOR_LOW for v in feat_vals]
                        ...
                        fig_shap.add_vline(x=0, line_color="#444", line_width=1)
                        fig_shap.update_layout(
                            **PLOTLY_THEME,
                            title=f"SHAP values — predicted class: {pred_label}  (red=pushes toward, green=pushes away)",
                        ...
                        st.markdown('<div class="section-title">Feature contribution summary</div>', unsafe_allow_html=True)
                        for i in top_idx[:5]:
                            fname = feature_cols[i]
                            fval  = float(X_input[0][i])
                            sv_i  = sv[i]
                            direction = "pushed toward" if sv_i > 0 else "pushed away from"
                            strength  = "strongly" if abs(sv_i) > 0.1 else "slightly"
                            st.markdown(f"- **{fname}** = `{fval:.2f}` → {strength} {direction} **{pred_label}** (SHAP: {sv_i:+.4f})")
                    else:
                        pass  # warning already shown in except block

                else:
                    st.warning("SHAP computation failed. Try Random Forest or XGBoost.")

            # ── Rule-based interpretation ────────────────────────────────────
            st.markdown('<div class="section-title">Signal interpretation</div>', unsafe_allow_html=True)
            reasons = []
            if is_stationary:
            ...
            if is_braking:
                reasons.append("🟡 High deceleration at low speed — stop-and-go traffic")
            if is_rush_hour:
            ...
            if not reasons:
                reasons.append("🔵 No dominant single signal — prediction from combined feature pattern")
            for r in reasons:
                st.markdown(f"- {r}")
