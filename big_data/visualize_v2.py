...
CONG_MAP = {0: "#3fb950", 1: "#ffa657", 2: "#f78166"}   # low / medium / high
CONG_LBL = {0: "Low",      1: "Medium",    2: "High"}
...
def fig_kinematics(df: pd.DataFrame):
    ...
    ax = axes[1, 2]
    bucket_labels = ["Night\n0-6", "Morning\n6-12", "Afternoon\n12-18", "Evening\n18-24"]
    # include_lowest=True captures hour==0; guards against empty bins crashing violinplot
    ...
    for b in bucket_labels:
        vals = df[df["hour_bucket"] == b]["speed"].dropna().values
        if len(vals) >= 2:
            vp_data.append(vals)
            vp_labels.append(b)
    if vp_data:
        positions = list(range(len(vp_data)))
        vp = ax.violinplot(vp_data, positions=positions, showmedians=True)
        ...
        ax.set_xticklabels(vp_labels, fontsize=8)
    else:
