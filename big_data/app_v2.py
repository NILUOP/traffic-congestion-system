        ...
        if st.button("▶  PREDICT CONGESTION LEVEL"):
            ...
            if not SHAP_AVAILABLE:
                ...
                if shap_vals is not None:
                    # shap_vals shape for RF multiclass: list of (1, n_features) per class
                    # Pick the predicted class
                    if isinstance(shap_vals, np.ndarray) and shap_vals.ndim == 3:
                        sv = shap_vals[pred][0]          # XGBoost format
                    elif isinstance(shap_vals, list):
                        sv = shap_vals[pred][0]          # RF format
                    else:
                        sv = shap_vals[0]
                    ...
                    fig_shap.update_layout(
                        **PLOTLY_THEME,
                        title=f"SHAP values for predicted class: {pred_label}  (red=pushes toward {pred_label}, green=pushes away)",
                    ...
                    st.warning("SHAP computation failed for this model. Try Random Forest or XGBoost.")
